   ...
   public class MyBlueToothChannel
    {
        ...
        public string Decoding(string uuid) {
            ...
            if (match.Success)
            {
                var hex = $"{match.Groups[1]}{match.Groups[2]}{match.Groups[3]}{match.Groups[4]}{match.Groups[5]}";
                ...
                return Encoding.ASCII.GetString(raw).Replace("\0", "");
            }
            return string.Empty;
        }
        ...
        {
            var hexData = BitConverter.ToString(Encoding.ASCII.GetBytes(data)).ToLower()
            .Replace("-", string.Empty).PadLeft(26, '0');
            String uuidString =
